...
class PartsController < ApplicationController
  ...
  def index
    ...
    if comp_sec? || vp_eng? || pur_eng? || ceo? || coo?
      @parts = Part.order("created_at DESC").paginate(:per_page => 40, :page => params[:page])    
    else
      @parts = Part.where("stock_qty > ?", 0).order("created_at DESC").paginate(:per_page => 40, :page => params[:page])
    end
  end
